...
#############################################
# simple dictionary for per year observations 

'''
year_dict = {x:[] for x in range(data.index.min().year,data.index.max().year+ 1)}


step = data.index.min().year
f=True
for i in range(len(total_time)-1) :
    if(total_time[i].year == total_time[i+1].year):
        year_dict[step].append(total_time[i])
        f=True
    else :
        # add the remaining same year observation m
        if(f==True) : year_dict[step].append(total_time[i])
        step+=1
        f=False
